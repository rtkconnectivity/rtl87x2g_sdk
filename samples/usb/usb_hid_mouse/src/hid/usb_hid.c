...
uint8_t HID_PROTOCOL = HID_REPORT_PROTOCOL;
//uint8_t IDLE_RATE = 0;
...
int usb_hid_get_report(uint8_t report_id, void *buf, uint16_t *len)
{
...
int usb_hid_set_report(void *buf, uint16_t len)
{
    int ret = 0;
    uint8_t *p_data = (uint8_t *)buf;
...
int usb_hid_get_protocol(uint8_t *Protocol)
{
    int ret = 0;
...
int usb_hid_set_protocol(uint8_t Protocol)
{
    int ret = 0;
...
int usb_hid_get_idle(uint8_t *idle_rate)
{
    int ret = 0;
    *idle_rate = 4;
...
int usb_hid_set_idle(uint8_t idle_rate)
{
    int ret = 0;
