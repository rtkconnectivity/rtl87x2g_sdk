...
:: -------------------------------------------------------------------------
:: Copy CMSE Lib (Up 4 levels)
:: -------------------------------------------------------------------------
set DEST_DIR=..\..\..\nsc\lib

if exist "%DEST_DIR%app_s_CMSE_Lib.o" (
    del /q "%DEST_DIR%app_s_CMSE_Lib.o"
)

if exist "%TARGET_OBJ_PATH%\app_s_CMSE_Lib.o" (
    echo Copying app_s_CMSE_Lib.o
    copy /Y "%TARGET_OBJ_PATH%\app_s_CMSE_Lib.o" "%DEST_DIR%"
) else (
    echo [WARNING] app_s_CMSE_Lib.o not found in Objects folder! Build might be incomplete.
)

1>NUL del   ..\..\..\..\..\tools\MergeTraceTool\input\%linker_output_file_name%.trace /f /q

1>NUL copy  .\bin\%linker_output_file_name%.trace                ..\..\..\..\..\tools\MergeTraceTool\input