...
#if (ENABLE_HW_AES_VERIFY==1)
#include "mbedtls_hw_interface.h"

void key_swap(uint8_t key[32])
{
    uint32_t tmp[4];
                          ...
                          unsigned char output[16])
{
    ...
    {
        uint8_t key_tmp[32];
        memcpy(key_tmp, (uint8_t *)ctx->rk, 32);
        key_swap(key_tmp);

        if (mode == MBEDTLS_AES_ENCRYPT)
        {
            return !(hw_aes_ecb256_encrypt((uint8_t *)input, key_tmp, output));
        }
        else
        {
            return !(hw_aes_ecb256_decrypt((uint8_t *)input, key_tmp, output));
        }
    }
    else
    {
        ...
        if (mode == MBEDTLS_AES_ENCRYPT)
        {
            return !(hw_aes_ecb128_encrypt((uint8_t *)input, (uint8_t *)ctx->rk, output));
        }
        else
        {
            return !(hw_aes_ecb128_decrypt((uint8_t *)input, (uint8_t *)ctx->rk, output));
        }
    }
 ...
 */
static const unsigned char aes_test_ecb_dec[2][16] =
{
    {
...
static const unsigned char aes_test_ecb_enc[2][16] =
{
    {
 ...
 */
static const unsigned char aes_test_cfb128_key[2][32] =
{
    {
...
static const unsigned char aes_test_cfb128_ct[2][64] =
{
    {
 ...
 */
static const unsigned char aes_test_ofb_key[2][32] =
{
    {
        0x2B, 0x7E, 0x15, 0x16, 0x28, 0xAE, 0xD2, 0xA6,
        0xAB, 0xF7, 0x15, 0x88, 0x09, 0xCF, 0x4F, 0x3C
...
static const unsigned char aes_test_ofb_ct[2][64] =
{
    {
        0x3B, 0x3F, 0xD9, 0x2E, 0xB7, 0x2D, 0xAD, 0x20,
        0x33, 0x34, 0x49, 0xF8, 0xE8, 0x3C, 0xFB, 0x4A,
...
static const unsigned char aes_test_ctr_key[2][16] =
{
    {
...
static const unsigned char aes_test_ctr_pt[2][48] =
{
    {
...
static const unsigned char aes_test_ctr_ct[2][48] =
{
    {
...
static const int aes_test_ctr_len[2] =
{ 16, 36 };
#endif /* MBEDTLS_CIPHER_MODE_CTR */
...
int mbedtls_aes_self_test(int verbose)
{
     ...
     * CFB128 mode
     */
    int aes_test_cfb128_key_len[2] = {0, 2};
    for (i = 0; i < 4; i++)
    {
        u = i >> 1;
        keybits = 128 + u * 128;
        ...
        memcpy(key, aes_test_cfb128_key[u], 16 + aes_test_cfb128_key_len[u] * 8);
