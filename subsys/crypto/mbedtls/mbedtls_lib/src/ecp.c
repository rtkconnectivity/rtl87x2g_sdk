                         ...
                         const mbedtls_ecp_point *P, const mbedtls_ecp_point *Q)
{
    ...
    hw_ecp_point result;
    ret = hw_ecp_compute(&result, (ECC_X_RESULT_ADDR), RTK_PUBKEY_ECC_ADD_POINT_ENTRY);
...
int mbedtls_ecp_add_point_test(int (*myrand)(void *, unsigned char *, size_t))
{
    int ret = 0;
    mbedtls_ecp_group grp;
    mbedtls_ecp_point add_result, mul_result;
    mbedtls_mpi m;

    mbedtls_ecp_group_init(&grp);
    mbedtls_ecp_point_init(&add_result);
    mbedtls_ecp_point_init(&mul_result);
    mbedtls_mpi_init(&m);

    mbedtls_ecp_group_load(&grp, MBEDTLS_ECP_DP_SECP256R1);

    MBEDTLS_MPI_CHK(ecp_add_mixed(&grp, &add_result, &grp.G, &grp.G));

    mbedtls_printf("ecp_add_mixed end");

    MBEDTLS_MPI_CHK(mbedtls_mpi_lset(&m, 2));
    MBEDTLS_MPI_CHK(mbedtls_ecp_mul(&grp, &mul_result, &m, &grp.G, myrand, NULL));

    mbedtls_printf("mbedtls_ecp_mul end");

    MBEDTLS_MPI_CHK(mbedtls_ecp_point_cmp(&add_result, &mul_result));
...
cleanup:
    ...
    if (ret != 0)
    {
        mbedtls_printf("Unexpected error, return code =  -0x%04x\n", -ret);
    }

    mbedtls_ecp_group_free(&grp);
    mbedtls_ecp_point_free(&add_result);
    mbedtls_ecp_point_free(&mul_result);
    mbedtls_mpi_free(&m);
