...
#if ENABLE_HW_ECC_VERIFY == 1 || ENABLE_HW_EDDSA_VERIFY == 1
#include <stdbool.h>
                          ...
                          const mbedtls_ecp_point *P, const mbedtls_ecp_point *Q )
{
    ...
    hw_ecc_init(mbedtls_mpi_bitlen(&grp->P), ECC_PRIME_MODE, GO_TO_END_LOOP, !RR_MOD_N_READY);
    hw_ecc_set_sub_operand((ECC_N_ADDR), grp->P.p, mbedtls_mpi_size(&grp->P));
    hw_ecc_compute(temp, (ECC_X_RESULT_ADDR), RTK_PUBKEY_N_INV_ENTRY);

    hw_ecc_init(mbedtls_mpi_bitlen(&grp->P), ECC_PRIME_MODE, GO_TO_END_LOOP, !RR_MOD_N_READY);
    hw_ecc_compute(temp, (ECC_X_RESULT_ADDR), RTK_PUBKEY_R_SQAR_ENTRY);
    ...
    hw_ecc_set_sub_operand((ECC_CURVE_A_ADDR), A_mod_P.p, mbedtls_mpi_size(&A_mod_P));
    hw_ecc_set_sub_operand((ECC_CURVE_B_ADDR), grp->B.p, mbedtls_mpi_size(&grp->B));

    hw_ecc_set_sub_operand(ECC_X_ADDR, P->X.p, mbedtls_mpi_size(&P->X));
    hw_ecc_set_sub_operand(ECC_Y_ADDR, P->Y.p, mbedtls_mpi_size(&P->Y));
    hw_ecc_set_sub_operand(ECC_Z_ADDR, P->Z.p, mbedtls_mpi_size(&P->Z));

    hw_ecc_set_sub_operand(ECC_X_RESULT_ADDR, Q->X.p, mbedtls_mpi_size(&Q->X));
    hw_ecc_set_sub_operand(ECC_Y_RESULT_ADDR, Q->Y.p, mbedtls_mpi_size(&Q->Y));
    hw_ecc_set_sub_operand(ECC_Z_RESULT_ADDR, Q->Z.p, mbedtls_mpi_size(&Q->Z));

    hw_ecp_point result;
    int ret = hw_ecc_compute(&result, (ECC_X_RESULT_ADDR), RTK_PUBKEY_ECC_ADD_POINT_ENTRY);
             ...
             mbedtls_ecp_restart_ctx *rs_ctx )
{
    ...
    if( mbedtls_ecp_get_type( grp ) == MBEDTLS_ECP_TYPE_MONTGOMERY )
    {
        ...
        hw_ecp_group ecc_group =
        {
            ...
            .key_bits = grp->pbits,
            .mode = ECC_MONTGOMERY_CURVE,
        };

        for (uint8_t i = 0; i < grp->P.n; i++)
        {
            ecc_group.A[i] = (i < grp->A.n) ? grp->A.p[i] : 0;
        ...
        hw_ecc_init(ecc_group.key_bits, ecc_group.mode, !GO_TO_END_LOOP, !RR_MOD_N_READY);

        if (hw_ecc_set_all_operands(&ecc_group, (uint32_t *)m->p, mbedtls_mpi_size(m)) == false)
        {
            return -1;
        }

        ret = hw_ecc_compute(&result, (ECC_X_RESULT_ADDR), RTK_PUBKEY_ECC_MUL_ENTRY);
