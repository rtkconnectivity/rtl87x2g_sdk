 ...
 *                              Constants
 *============================================================================*/
/** @defgroup DFU_SERVICE_Exported_Constants DFU service Exported Constants
  * @{
  */
...
/** End of DFU_SERVICE_Exported_Constants
  * @}
  */
 ...
 *                              Variables
 *============================================================================*/
/** @defgroup DFU_SERVICE_Exported_Variables DFU service Exported Variables
  * @brief
  * @{
  */

/** @brief  Service ID */
T_SERVER_ID dfu_srv_id_local;


/** @brief  Function pointer used to send event to application from DFU service
*   @note   It is initiated in dfu_add_service()
*/
#if F_APP_GATT_SERVER_EXT_API_SUPPORT
static P_FUN_EXT_SERVER_GENERAL_CB p_dfu_extended_cb = NULL;
#else
P_FUN_SERVER_GENERAL_CB p_dfu_extended_cb = NULL;
#endif

/** End of DFU_SERVICE_Exported_Variables
  * @}
  */
/** End of DFU_SERVICE
  * @}
                                              ...
                                              uint8_t *p_value, P_FUN_WRITE_IND_POST_PROC *p_write_ind_post_proc)
{
    ...
    callback_data.msg_data.write.write_attrib_index = attr_index;
...
//    app_reg_le_link_disc_cb(conn_id, dfu_service_le_disconnect_cb);
    ...
    {
        ble_dfu_service_handle_packet(conn_id, length, p_value);
    }
    else if (INDEX_DFU_CHAR_DFU_CONTROL_POINT_INDEX == attr_index)
    {
        ble_dfu_service_handle_cp_req(conn_id, length, p_value);
    }
    else
    {
 ...
 * @param    conn_id   Connection ID used to identify the specific connection.
 * @param    p_data    Pointer to the data to be sent as a notification.
